...
function resolveTierByAmount(amountCents) {
  if (amountCents === null) {
    return TIERS.STANDARD;
  }

  const foundersMin = Number(process.env.TIER_MIN_CENTS_FOUNDERS || 2000000);
  const eliteMin = Number(process.env.TIER_MIN_CENTS_ELITE || 420000);
  const earlyMin = Number(process.env.TIER_MIN_CENTS_EARLY || 100000);
  const midMin = Number(process.env.TIER_MIN_CENTS_MID || 50000);
...
function inferTierFromNames(payload) {
  const itemProductName = normalizeText(get(payload, 'data.attributes.first_order_item.product_name'));
  const itemVariantName = normalizeText(get(payload, 'data.attributes.first_order_item.variant_name'));
  const attrProductName = normalizeText(get(payload, 'data.attributes.product_name'));
  const attrVariantName = normalizeText(get(payload, 'data.attributes.variant_name'));
  const candidates = [itemProductName, itemVariantName, attrProductName, attrVariantName].filter(Boolean);

  for (const text of candidates) {
    if (text.includes('#1-10')) return TIERS.FOUNDERS;
    if (text.includes('#11-20')) return TIERS.ELITE;
    if (text.includes('#21-50')) return TIERS.EARLY;
    if (text.includes('#51-100')) return TIERS.MID;
    if (text.includes('#101+')) return TIERS.STANDARD;
  }
...
function resolveTierFromPayload(payload) {
  ...
  const inferredTier = inferTierFromNames(payload);
  if (inferredTier) {
    return inferredTier;
  }
  ...
  return resolveTierByAmount(amountCents);
}
...
function resolveNameAddonPaid(payload) {
  ...
  const productName = normalizeText(get(payload, 'data.attributes.first_order_item.product_name'));
  const variantName = normalizeText(get(payload, 'data.attributes.first_order_item.variant_name'));
  if (
    productName.includes('with name') ||
    productName.includes('name add-on') ||
    variantName.includes('with name') ||
    variantName.includes('name add-on')
  ) {
    return true;
  }
...
function extractWebhookFields(payload) {
  ...
  const name = cleanString(
    ...
    get(payload, 'meta.custom_data.displayName') ||
    get(payload, 'meta.custom_data.name') ||
    get(payload, 'meta.display_name') ||
    get(payload, 'meta.name') ||
    get(payload, 'data.attributes.display_name') ||
    get(payload, 'data.attributes.name') ||
    get(payload, 'data.attributes.custom_data.display_name') ||
    get(payload, 'data.attributes.custom_data.name') ||
...
function extractStripeFields(eventPayload) {
  const session = get(eventPayload, 'data.object') || {};
  const customFields = Array.isArray(session.custom_fields) ? session.custom_fields : [];

  let customDisplayName = null;
  for (const field of customFields) {
    if (!field || typeof field !== 'object') continue;
    const key = normalizeText(field.key || field.label && String(field.label.custom || field.label || ''));
    if (!key.includes('display') && !key.includes('name')) continue;
    if (field.type === 'text') {
      customDisplayName = cleanString(get(field, 'text.value') || field.value);
      if (customDisplayName) break;
    }
  }

  const orderId = normalizeOrderReference(
    session.id ||
  ...
  const orderNumber = normalizeOrderReference(
    session.payment_intent ||
    session.invoice ||
    session.client_reference_id ||
    session.id
  ...
  const email = cleanString(
    ...
    get(eventPayload, 'data.object.customer_email')
  );

  const name = cleanString(
    get(session, 'metadata.display_name') ||
    get(session, 'metadata.displayName') ||
    get(session, 'metadata.name') ||
    get(session, 'customer_details.name') ||
    customDisplayName
  );

  return { orderId, orderNumber, email, name };
}

function resolveTierFromStripeSession(eventPayload) {
  const session = get(eventPayload, 'data.object') || {};
  const metadataTier = normalizeTier(
    get(session, 'metadata.tier') ||
    get(session, 'metadata.hat_tier') ||
    get(session, 'client_reference_id')
  );
  if (metadataTier) {
    return metadataTier;
  }
  ...
  const idCandidates = asStringSet([
    ...
    get(session, 'metadata.payment_link_id'),
    get(session, 'metadata.link_id'),
    get(session, 'metadata.price_id'),
    get(session, 'metadata.variant_id')
  ]);

  const envTierMatches = {
    [TIERS.FOUNDERS]: asStringSet([
      ...parseIdList(process.env.STRIPE_PAYMENT_LINK_ID_FOUNDERS),
      ...parseIdList(process.env.STRIPE_PRICE_ID_FOUNDERS)
    ]),
    [TIERS.ELITE]: asStringSet([
      ...parseIdList(process.env.STRIPE_PAYMENT_LINK_ID_ELITE),
      ...parseIdList(process.env.STRIPE_PRICE_ID_ELITE)
    ]),
    [TIERS.EARLY]: asStringSet([
      ...parseIdList(process.env.STRIPE_PAYMENT_LINK_ID_EARLY),
      ...parseIdList(process.env.STRIPE_PRICE_ID_EARLY)
    ]),
    [TIERS.MID]: asStringSet([
      ...parseIdList(process.env.STRIPE_PAYMENT_LINK_ID_MID),
      ...parseIdList(process.env.STRIPE_PRICE_ID_MID)
    ]),
    [TIERS.STANDARD]: asStringSet([
      ...parseIdList(process.env.STRIPE_PAYMENT_LINK_ID_STANDARD),
      ...parseIdList(process.env.STRIPE_PRICE_ID_STANDARD)
    ])
  };
  ...
  const tierOrder = [TIERS.FOUNDERS, TIERS.ELITE, TIERS.EARLY, TIERS.MID, TIERS.STANDARD];
  for (const tier of tierOrder) {
    const mappedIds = envTierMatches[tier];
    if (!mappedIds.size) continue;
    for (const candidate of idCandidates) {
      if (mappedIds.has(candidate)) {
        return tier;
      }
    }
  }
  ...
  const amountCents = parseMoneyCents(
    session.amount_subtotal !== undefined ? session.amount_subtotal : session.amount_total
  );
  return resolveTierByAmount(amountCents);
}
