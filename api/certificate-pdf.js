...
function safeFilenamePart(value) {
  return String(value || 'hat-certificate').replace(/[^a-z0-9_-]/gi, '-').toLowerCase();
}
...
module.exports = async (req, res) => {
  ...
  let pdfBuffer;
  try {
    pdfBuffer = await buildCertificatePdf(certificate);
  } catch (err) {
    return res.status(500).json({ error: 'PDF generation failed' });
  }

  const fileKey = safeFilenamePart(certificateNumber);
